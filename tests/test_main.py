...
class TestCatchTime(IsolatedAsyncioTestCase):
    ...
    async def test_basic_usage(self):
        ...
        self.assertAlmostEqual(ct.elapsed, self.wait, delta=0.04)
    ...
    async def test_logger(self):
        ...
        # Use the logger
        with timer(logger=logger) as ct:
            await asyncio.sleep(self.wait)
        await asyncio.sleep(self.wait_after)

        # Check the output
        self.assertEqual(stream.getvalue(), f"Elapsed time: {ct.elapsed:.3f}\n")
    ...
    async def test_wrapped_async(self):
        ...
        self.assertAlmostEqual(float(stream.getvalue()), self.wait, delta=0.04)
